"use client";

import { signIn, signOut } from "next-auth/react";
import { useState } from "react";
import { Button } from "@/components/ui/button";
import { Loader2 } from "lucide-react";
import { useSearchParams } from "next/navigation";

export function GoogleSignInButton({ callbackUrl, role: propRole }: { callbackUrl?: string; role?: string | null }) {
    const [isLoading, setIsLoading] = useState(false);
    const searchParams = useSearchParams();
    const urlRole = searchParams.get("role");
    const role = propRole !== undefined ? propRole : urlRole;

    const handleSignIn = async () => {
        setIsLoading(true);
        try {
            let finalCallbackUrl = callbackUrl || "/proxy";
            if (role) {
                const url = new URL(finalCallbackUrl, window.location.origin);
                url.searchParams.set("role", role);
                finalCallbackUrl = url.pathname + url.search;
            }
            await signIn("google", { callbackUrl: finalCallbackUrl });
        } catch (error) {
            console.error("Google sign in failed:", error);
            setIsLoading(false);
        }
    };

    return (
        <Button
            variant="outline"
            className="w-full flex items-center justify-center gap-2 border-neutral-200 hover:bg-neutral-50 dark:border-neutral-800 dark:hover:bg-neutral-800/50 py-5 rounded-xl text-primary-900 transition-colors text-sm font-semibold cursor-pointer"
            onClick={handleSignIn}
            disabled={isLoading}
        >
            {isLoading ? (
                <Loader2 className="w-5 h-5 animate-spin text-neutral-500" />
            ) : (
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path
                        fill="#4285F4"
                        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                    />
                    <path
                        fill="#34A853"
                        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    />
                    <path
                        fill="#FBBC05"
                        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
                    />
                    <path
                        fill="#EA4335"
                        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    />
                </svg>
            )}
            <span>Masuk dengan Google</span>
        </Button>
    );
}

export function GitHubSignInButton({ callbackUrl, role: propRole }: { callbackUrl?: string; role?: string | null }) {
    const [isLoading, setIsLoading] = useState(false);
    const searchParams = useSearchParams();
    const urlRole = searchParams.get("role");
    const role = propRole !== undefined ? propRole : urlRole;

    const handleSignIn = async () => {
        setIsLoading(true);
        try {
            let finalCallbackUrl = callbackUrl || "/proxy";
            if (role) {
                const url = new URL(finalCallbackUrl, window.location.origin);
                url.searchParams.set("role", role);
                finalCallbackUrl = url.pathname + url.search;
            }
            await signIn("github", { callbackUrl: finalCallbackUrl });
        } catch (error) {
            console.error("GitHub sign in failed:", error);
            setIsLoading(false);
        }
    };

    return (
        <Button
            variant="outline"
            className="w-full flex items-center justify-center gap-2 border-neutral-200 hover:bg-neutral-50 dark:border-neutral-800 dark:hover:bg-neutral-800/50 py-5 rounded-xl text-primary-900 transition-colors text-sm font-semibold cursor-pointer"
            onClick={handleSignIn}
            disabled={isLoading}
        >
            {isLoading ? (
                <Loader2 className="w-5 h-5 animate-spin text-neutral-500" />
            ) : (
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.167 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.577.688.479C19.138 20.164 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
                </svg>
            )}
            <span>Masuk dengan GitHub</span>
        </Button>
    );
}

export function SignOutButton() {
    const [isLoading, setIsLoading] = useState(false);

    const handleSignOut = async () => {
        setIsLoading(true);
        try {
            await signOut({ callbackUrl: "/" });
        } catch (error) {
            console.error("Sign out failed:", error);
            setIsLoading(false);
        }
    };

    return (
        <Button
            variant="ghost"
            className="flex items-center gap-1.5 hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-600 dark:text-neutral-300 font-semibold px-4 py-2 rounded-xl text-sm transition-colors cursor-pointer"
            onClick={handleSignOut}
            disabled={isLoading}
        >
            {isLoading && <Loader2 className="w-4 h-4 animate-spin" />}
            <span>Keluar</span>
        </Button>
    );
}